...
def get_bytes_range(source: PathOrStr, bytes_start: int, num_bytes: int) -> bytes:
    if bytes_start <= 10000: # debug: are we actually reading from the contamination file?
        log.info(f"Reading some initial bytes of file {source}")
...
def _http_get_bytes_range(scheme: str, host_name: str, path: str, bytes_start: int, num_bytes: int) -> bytes:
    ...
    if len(result) >  num_bytes: # let us fix this issue
        print(f"expected {num_bytes} bytes, got {len(result)}. Taking the first {num_bytes} bytes")
